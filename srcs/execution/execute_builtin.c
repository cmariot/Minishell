...
/*   Updated: 2022/01/18 15:32:12 by cmariot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	command_is_builtin(t_shell **minishell, t_simple command, int *backup_fd)
{
	...
	if (file_redirection(&fd_redir[0], &fd_redir[1], command))
		return (0);
	if (ft_strcmp(command.command_and_args[0], "cd") == 0)
		...
		return (its_not_builtin(command, fd_redir));
	restore_file_redirection(command, fd_redir[0], fd_redir[1], TRUE);
	return (0);
}
